...
function isCommandTimeRestricted(userGroup, command, controller) {
  if (controller.connections.database.timeRestrictionDisabled || !(command.name in timeRestrictedCommands)) {
 ...
 * @returns {boolean} 
 */
module.exports = (user, command, controller) => {
  const userGroup = groupMemberships[user];

  return doesUserHaveAccessToCommand(user, userGroup, command.permission) &&

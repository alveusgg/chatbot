  ...
  return {
    ...
    run: async ({ channel, args }) => {
      ...
      setTimeout(async () => {
        ...
        logger.log('Change Server Status:', serverName, cloudLive, obs.cloud.name);

        const message = cloudLive ? `Cloud Server changed to ${serverName}` : `Cloud Server-${serverName} offline`;

    ...
    run: async ({ args: _args }) => {
      ...
      if (specificCamera) {
        if (args[1] !== '') {
          ...
          database[currentScene].presets[specificCamera] = currentScene;
        }
      } else {
        if (args[1] !== '') {
          database[currentScene].presets[args1] = currentScene;
        } else {
          database[currentScene].lastKnownPosition = currentScene;
        }
      }

      await api.sendBroadcastMessage(`ptzsave ${specificCamera} ${args[1]}`, 'backend');

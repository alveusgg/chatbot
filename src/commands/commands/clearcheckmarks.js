...
const { roundsCommandMapping, customCommandAlias } = require('../../config/config.js');
  ...
  return {
    ...
    run: async ({ args }) => {
      ...
      if (arg1Base in roundsCommandMapping) {
        await obs.local.setSceneItemEnabled('RoundsOverlay', roundsCommandMapping[arg1Base], checkmarkStatus);
      } else {
        for (const source in roundsCommandMapping) {
          await obs.local.setSceneItemEnabled('RoundsOverlay', roundsCommandMapping[source], checkmarkStatus);
        }
      }
    }
  }

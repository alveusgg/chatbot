...
const { customCommandAlias, axisCameraCommandMapping } = require('../../config/config.js');
      ...
      switch (arg1) {
        ...
        default: {
          for (let i = 0; i < currentCamList.length; i++) {
            ...
            const baseName = customCommandAlias[ptzCameraName] ?? ptzCameraName;

            ptzCameraName = axisCameraCommandMapping[baseName] ?? baseName;

            output += `${i + 1}: ${ptzCameraName}`;

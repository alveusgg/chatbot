...
module.exports = async (controller, name, oldName) => {
  ...
  if (!pauseNotify) {
    ...
    if (hour >= notifyHours.start && hour < notifyHours.end) {
      ...
      if (continueCheck) {
        logger.log("Notify: Not multiScenes", notifyScenes, newScene, oldScene);
  ...
  if (!pauseGameChange) {
    ...
    if (justChattingCams.includes(name)) {
      await controller.connections.twitch.setStreamInfo(alveusTwitchID, null, "chatting");
    } else if (poolsCam.includes(name)) {
      await controller.connections.twitch.setStreamInfo(alveusTwitchID, null, "pools");
    } else {
      await controller.connections.twitch.setStreamInfo(alveusTwitchID, null, "animals");
    }
  }
  ...
  if (!pauseTwitchMarker) {
    ...
    if (markerCams.includes(name)) {
      //new swap
      marker = await controller.connections.twitch.createMarker(alveusTwitchID, `Livecam Switched to ${name}`);

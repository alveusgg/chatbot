...
const { pauseTwitchMarker, alveusTwitchID, announceChatSceneChange } = require('../config/config.js');
...
module.exports = async (controller, name, oldName) => {
  ...
  if (!pauseTwitchMarker) {
    ...
    if (markerCams.includes(name)) {
      ...
      //ending swap
      marker = await controller.connections.twitch.createMarker(config.alveusTwitchID, `Livecam Switched to ${name}`);
    }

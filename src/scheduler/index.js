...
class Scheduler {
  ...
  async loadTasks() {
    const files = (await getAllFiles(join(__dirname, 'tasks'))).filter(path => path.endsWith('.js'));
...
function assertTask(file, task) {
  if (typeof task !== 'object') {
    throw new TypeError(`${file}: expected task to be an object, got ${typeof task}`);
  }

  if (typeof task.name !== 'string') {
    throw new TypeError(`${file}: expected name to be a string, got ${typeof task.name}`)
  }

  if (typeof task.enabled !== 'boolean') {
    throw new TypeError(`${file}: expected enabled to be a boolean, got ${typeof task.enabled}`)
  }

  if (typeof task.runAt !== 'object') {
    throw new TypeError(`${file}: expected runAt to be an object, got ${typeof task.runAt}`);
  }

  if (typeof task.runAt.hour !== 'number') {
    throw new TypeError(`${file}: expected runAt.hour to be a number, got ${typeof task.runAt.hour}`)
  }

  if (typeof task.runAt.minute !== 'number') {
    throw new TypeError(`${file}: expected runAt.minute to be a number, got ${typeof task.runAt.minute}`)
  }
